...
Education functions as a potential covariate since individuals with higher levels of education are more likely to own technology and have improved economic outcomes.
...
One empirical extension for this project would be to restrict the data to urban respondents. Although the main model includes urban/rural residence as a covariate, using subgroup analysis would help isolate the effect of the technology specifically in urban settings (rather than just controlling for urban/rural group status).

This distinction is crucial because for variables like Q90i — which measures how frequently individuals use the internet — usage could either be influenced by broader internet access issues in a village (typical of a rural setting), or reflect an individual's income level. Differences that are manifested through ones location impact other variables in the model such as digital access in unique ways.
...
One issue with this research design is that it carries inevitable concerns regarding reverse causality. In particular, because the Afrobarometer survey data used for this project is cross-sectional and represents only a single point in time, it can not establish a clear direction of causality.
...
The table above presents the results of an OLS regression estimating the effects of digital access on poverty levels, while controlling for corruption, education, and urban/rural residence. I referred to Claude for guidance in this section to learn about how the coefficients change in the presence of an interaction term.
